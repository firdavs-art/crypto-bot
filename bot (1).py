...
async def ask_groq(user_msg: str, crypto_prices: dict, stock_prices: dict) -> str:
    ...
    # Retry up to 4 times on rate limit (429), with increasing wait
    ...
    for attempt, wait in enumerate(wait_times, 1):
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if r.status_code == 429:
                    logger.warning(f"Groq rate limited — waiting {wait}s (attempt {attempt}/4)")
            ...
            if attempt == len(wait_times):
                return f"❌ AI request failed after retries: {e}"
            await asyncio.sleep(wait)

    return "❌ AI is rate limited right now — please wait 1 minute and try again."
