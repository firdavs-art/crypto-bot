...
def fetch_stock_price(ticker: str) -> dict:
    try:
        ...
        if len(hist) >= 2:
            ...
            price = float(hist["Close"].iloc[-1])
            return {"price": round(price, 2), "change24h": 0.0}
    except Exception as e:
...
async def _run_briefing(context: ContextTypes.DEFAULT_TYPE, reply_msg=None):
    ...
    # Strict limits to keep payload small and avoid 413
    world  = "\n".join(fetch_rss(NEWS_FEEDS,   limit_per_feed=2)[:8])  or "No headlines"
    crypto = "\n".join(fetch_rss(CRYPTO_FEEDS, limit_per_feed=2)[:5])  or "No crypto news"

    # Compact portfolio: just symbol, price, pnl% — no names
    port_lines = []
    for sym, h in {**CRYPTO, **STOCK}.items():
        prices = {**cp, **sp}
        p = prices.get(sym, {})
        price = p.get("price", 0)
        chg   = p.get("change24h", 0)
        ...
        val   = price * h["qty"]
        pnl_p = ((val - cost) / cost * 100) if cost else 0
        port_lines.append(f"{sym}: {chg:+.1f}% pnl={pnl_p:+.0f}%")
    port_compact = "  ".join(port_lines)
    ...
    prompt = (
        ...
        f"CRYPTO: {crypto}\n"
        f"MOVERS: {movers}\n"
        f"PORTFOLIO: {port_compact}\n\n"
        "Write:\n"
        "🌍 GEOPOLITICAL: 2 bullets (US/EU/Russia/China only)\n"
        "📊 MACRO: 2 bullets (CPI/Fed/inflation if relevant)\n"
        "🪙 CRYPTO: 2 bullets (key news, my coins/BTDR)\n"
        "📈 TOP MOVERS: biggest moves with brief reason\n"
        "💡 INSIGHT: one sharp action point\n"
        "Max 200 words total."
    )
...
def main():
    ...
    # Schedule daily 7am UK briefing
    # chat_id passed via job_kwargs so the briefing knows where to send
    app.job_queue.run_daily(
        morning_briefing,
        time=dtime(hour=7, minute=0, second=0, tzinfo=UK_TZ),
        name="morning_briefing",
        job_kwargs={"misfire_grace_time": 3600},  # send even if bot was down, up to 1hr late
    )

    logger.info("🚀 Bot started! Daily briefing scheduled at 07:00 UK time.")
